...
export const DataciteApiLive = Layer.effect(
  ...
  Effect.gen(function* (_) {
    ...
    const client = httpClient.pipe(
      ...
      HttpClient.client.mapRequest(HttpClient.request.prependUrl('https://api.datacite.org/')),
    )
    ...
    const getWork = (doi: Doi.Doi) =>
      Effect.gen(function* (_) {
        const response = yield* _(
          HttpClient.request.get(`dois/${encodeURIComponent(doi)}`),
          okClient,
          Effect.flatMap(HttpClient.response.schemaBodyJson(DataSchema('dois', WorkSchema))),
        )
